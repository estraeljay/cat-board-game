...
- **Dice / movement (revised 2026-08-28)**: the roll (still a placeholder d6) sets a **maximum**. The player then **walks the path one tile at a time with the arrow keys or WASD, and presses Enter (or "Stop here") to end** — up to the max — so you can land exactly on a property/special entry within range (`awaiting-move` phase). On-screen direction buttons mirror the keys; the board highlights your tile (blue) and the tiles you can step to (gold). Snared (odd check die) still forces exactly 1 tile. See `MASTER-DATA-SHEET.md` §2.
